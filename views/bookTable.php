...
if (isset($_POST["submit"]) && !empty($_POST["search"])) {
  ...
  $sql = "SELECT * FROM books WHERE book_id LIKE '%$search%' OR title LIKE '%$search%' OR publish_date LIKE '%$search%' OR number_of_pages LIKE '%$search%' OR genre LIKE '%$search%' OR author_id LIKE '%$search%'";
} else {
...
$total_records_query = "SELECT COUNT(*) as count FROM books";
  ...
  <main>
    ...
    <div class="container mb-3">
      <div class="row mx-2 rounded-top-2">
        ...
        <h2 class="mb-3 mt-2" style="color:white">Book Table</h2>
        <!-- search bar -->
        <div class="col">
          <br>
        ...
        <div class="col text-end">
          <a href="./books/create-book.php" class="btn btn-color mb-1 mb-lg-0">Add new book<i class="bi bi-plus-circle ms-2"></i></a>
          <div class="btn-group">
      ...
      <div class="row mx-2 px-3 table-responsive">
        <table class="table table-hover" id="example">
          ...
          <tbody class="table-group-divider">
            <?php
            foreach ($books as $content) {
            ?>
              <tr>
                <td><?= $content['book_id'] ?></td>
                <td><?= $content['title'] ?></td>
                <td><?= $content['publish_date'] ?></td>
                <td><?= $content['number_of_pages'] ?></td>
                <td><?= $content['genre'] ?></td>
                <td><?= $content['author_id'] ?></td>
                <td>
                  <a href="./books/view-book.php?id=<?= $content['book_id'] ?>" class="btn btn-outline-secondary"><i class="bi bi-binoculars"></i></a>
                  <a href="./books/update-book.php?id=<?= $content['book_id'] ?>" class="btn btn-outline-success"><i class="bi bi-pencil-square"></i></a>
        ...
        <nav class="d-flex justify-content-center">
          <ul class="pagination">
            <li class="page-item">
              <a class="page-link" href="bookTable.php?page=<?= $previous ?>"><i class="bi bi-chevron-double-left"></i></a>
            </li>
            <?php for ($i = 1; $i <= $pages; $i++) : ?>
              <li class="page-item"><a class="page-link" href="bookTable.php?page=<?= $i ?>"><?= $i ?></a></li>
            <?php endfor; ?>
            <li class="page-item">
              <a class="page-link" href="bookTable.php?page=<?= $next ?>"><i class="bi bi-chevron-double-right"></i></a>
            </li>
          </ul>
        </nav>
      </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title"> Delete Book Data </h4>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <form action="./books/delete-book.php" method="POST">
          <div class="modal-body">
            <input type="hidden" name="book_id" id="delete_id">
            <h6>Do you want to delete this book?</h6>
          </div>
