...
if (isset($_POST['update_librarian'])) {
    ...
    if ($password != $repeatPassword) {
        ...
        if ($stmt->affected_rows > 0) {
            $_SESSION['message'] = "Librarian has been successfully updated!";
            ...
            <div class="row rounded-2 p-4 bg-white shadow">
                <?php if (isset($_SESSION['message'])) { ?>
                    <div class="alert alert-success mb-0 rounded-bottom-0" role="alert">
                        <div>
                            <i class="bi bi-person-add me-1"></i><?php echo $_SESSION['message']; ?>
                        </div>
                    </div>
                <?php
                ...
                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?id=' . $_GET['id']); ?>" method="POST" class="needs-validation" novalidate id="librarianForm">
                    ...
                    <div class="d-grid col-4 mx-auto my-3">
                        <button class="btn btn-outline-success btn-lg" type="submit" name="update_librarian" <?= $isAdmin ? 'disabled' : ''; ?>>Update Librarian</button>
                    </div>
                </form>
            </div>
        </div>
